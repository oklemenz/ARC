...
@objc
protocol VirtualObjectInteractionDelegate {
...
class VirtualObjectInteraction: NSObject, UIGestureRecognizerDelegate {
    ...
    weak var delegate: VirtualObjectInteractionDelegate?
    ...
    func didRotate(_ gesture: UIRotationGestureRecognizer) {
        guard delegate!.isInteractionMode() else { return }
    ...
    func didTap(_ gesture: UITapGestureRecognizer) {
        ...
        if let tappedObject = sceneView.virtualObject(at: touchLocation) {
            // Select a new object.
            if delegate?.isRemoveModeActive() == false {
                selectedObject = tappedObject.root
            } else {
                delegate?.didTapObjectForRemove(object: tappedObject)
            }
    ...
    func didDoubleTap(_ gesture: UITapGestureRecognizer) {
        ...
        if let doubleTappedObject = sceneView.virtualObject(at: touchLocation) {
            selectedObject = doubleTappedObject
            delegate?.didTapObjectForClone(object: doubleTappedObject)
        }
    }
